...
- MasterGo 未提供的可选字段必须删除整行属性；不得猜值、填空或新增代码库未登记字段。
...
- 无图标组件不得生成 `Icon`、`IconWidth`、`IconHeight`；没有图标槽位时不要在模板中保留这些属性。
- `FontSize` 与 `Height` 始终分开表达；不得用字号、行高或文本 bbox 高度替代控件 Height。
