...
新模式在用户要求项目部署、运行时重载或可运行页面时，目标是最终可运行页面，不是临时稿或“先能显示再补组件”的中间结果。用户只要求独立结构映射 XML 时，可以没有目标项目，但必须先完成正式组件映射，并按映射生成真实控件结构；不能因为运行时配置缺失而退化为无类型容器。
...
- 中间映射表可以生成，但不得把临时 XML 替换到目标页面作为交付。只有组件、样式、图标、IO 绑定、坐标、键查证、同步、目标项目重载与视觉核对全部通过，才能报告完成。
...
2. **页面注册固定模板**：按目标项目已确认的注册结构添加页面 Target 与菜单入口。示例模板：`<Page Target="{target}" LangName="{page_title_key}"/>`；菜单项仅在目标项目存在该结构时，按其固定字段模板填入 `PageName="Jump:{target}"`。改前备份实际注册文件。
