...
匹配键统一使用“父节点语义 + MasterGo 变量属性名 + 变量值”。例如：父节点=右侧栏 + 变量属性名=按钮类型 + 变量值=上下布局。组件集 ID、实例 ID 和设计师自定义名称只作为读取追踪信息，不参与唯一匹配；变量值负责切换固定实例模板，ControlType、节点数量、父子关系和槽位顺序由模板固定，花括号字段由对应 MasterGo 节点或业务配置填充。
